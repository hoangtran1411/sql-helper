...
func TestGitHubConstants(t *testing.T) {
	if GitHubOwner != "hoangtran1411" {
		t.Errorf("GitHubOwner = %q, want %q", GitHubOwner, "hoangtran1411")
	}
	if GitHubRepo != "sql-helper" {
		t.Errorf("GitHubRepo = %q, want %q", GitHubRepo, "sql-helper")
	}
}
